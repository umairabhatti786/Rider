import { parallaxLayout } from "./parallax"; // Mock react-native-reanimated
...
jest.mock("react-native-reanimated", () => ({
  interpolate: jest.fn((value, inputRange, outputRange) => {
    ...
    if (value >= inputMax) return outputMax; // Linear interpolation
...
describe("parallaxLayout", () => {
  ...
  it("should return transform object for vertical layout", () => {
    const verticalConfig = { ...baseConfig,
      vertical: true
    };
  ...
  it("should handle center position (value = 0)", () => {
    const layout = parallaxLayout(baseConfig);
    const result = layout(0); // At center position, translateX should be 0

    expect(result.transform[0].translateX).toBe(0); // Scale should be a number (parallax logic applied)

    expect(typeof result.transform[1].scale).toBe("number");
  });
  it("should handle left position (value = -1)", () => {
    const layout = parallaxLayout(baseConfig);
    const result = layout(-1); // At left position, translateX should be -size + parallaxScrollingOffset
  ...
  it("should handle right position (value = 1)", () => {
    const layout = parallaxLayout(baseConfig);
    const result = layout(1); // At right position, translateX should be size - parallaxScrollingOffset
  ...
  it("should handle custom parallaxScrollingScale", () => {
    ...
    const sideResult = layout(-1); // Verify the structure and that scale values are calculated
  ...
  it("should handle edge case values", () => {
    const layout = parallaxLayout(baseConfig); // Test with values beyond the normal range
  ...
  it("should handle zero size", () => {
    const zeroSizeConfig = { ...baseConfig,
      size: 0
    };
  ...
  it("should handle negative size", () => {
    const negativeSizeConfig = { ...baseConfig,
      size: -100
    };
  ...
  it("should handle all config properties at once", () => {
    ...
    const centerResult = layout(0);
    const sideResult = layout(-1); // Verify the structure and that values are calculated correctly
  ...
  it("should handle empty mode config", () => {
    const layout = parallaxLayout(baseConfig, {});
    const result = layout(0); // At center (value = 0), scale should be a number (default parallax behavior)
  ...
  it("should maintain consistent zIndex calculation", () => {
    ...
    expect(typeof rightResult.zIndex).toBe("number"); // Center should have highest zIndex
