...
describe("dealWithAnimation", () => {
  ...
  it("should handle spring animation with different values", () => {
    ...
    expect(withSpring).toHaveBeenCalledWith(-100, {
      damping: 15
    }, expect.any(Function));
    expect(withSpring).toHaveBeenCalledWith(500, {
      damping: 15
    }, expect.any(Function));
  ...
  it("should handle timing animation with different values", () => {
    ...
    expect(withTiming).toHaveBeenCalledWith(1000, {
      duration: 500
    }, expect.any(Function));
    expect(callback).toHaveBeenCalledTimes(3);
  });
  ...
  it("should handle fractional values", () => {
    ...
    expect(withSpring).toHaveBeenCalledWith(50.5, {
      damping: 10
    }, expect.any(Function));
    expect(withSpring).toHaveBeenCalledWith(-25.75, {
      damping: 10
    }, expect.any(Function));
  });
